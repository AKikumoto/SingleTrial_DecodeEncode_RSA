...
Dir_RSAMODEL<-file.path(Dir_HERE,"CONJ_v1")
...
names(mL_S)<-c("CONJ","RESP","RULE","STIM")
...
r<-foreach(i=1:ntrial) %dopar%{
  ...
  rule_s <- mL_S[["RULE"]][cl,] + rnorm(ncontext,0,.5);# runif(ncontext,min=0,max=1)
  stim_s <- mL_S[["STIM"]][cl,] + rnorm(ncontext,0,.5);# runif(ncontext,min=0,max=1)
  resp_s <- mL_S[["RESP"]][cl,] + rnorm(ncontext,0,.5);# runif(ncontext,min=0,max=1)
  conj_s <- mL_S[["CONJ"]][cl,] + rnorm(ncontext,0,.2);# runif(ncontext,min=0,max=1)
  noise <- data.table(t(rnorm(ncontext,0,1)));
  #noise <- data.table(t(runif(ncontext,min=0,max=1)));
  names(noise) <- paste0("NOISE_",names(noise)) 
  
  # Signal (Adjust this part!)
  obs <- cbind(rule_s,resp_s,noise); 
}
...
results<-
  foreach(cc=clist) %dopar% {
    ...
    predF$CONJ_RSA<-CONJ_M[,cc];#CONJ MODEL
    predF$CTR_RSA<-RAND_M[,cc];#RANDOM MODEL
    dvIdx<-grepl("CLASS|RULE_RSA|STIM_RSA|RESP_RSA|CONJ_RSA|CTR_RSA",colnames(predF))
