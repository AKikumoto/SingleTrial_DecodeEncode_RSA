# Conjunction_Simulate congruence
...
Dir_RSAMODEL<-file.path(Dir_HERE,"TSNCONJ")
...
names(mL_S)<-c("CONG","RESP","RESPN","RULE","STIM","TSCONJ","TSNCONJ_M")
...
r<-foreach(i=1:ntrial) %dopar%{
  ...
  respn_s <- mL_S[["RESPN"]][cl,] + rnorm(ncontext,0,0.5);
  cong_s <- mL_S[["CONG"]][cl,] + rnorm(ncontext,0,0.5);
  conj_low_s <- mL_S[["TSCONJ"]][cl,] + rnorm(ncontext,0,0.5);
  conj_high_s <- mL_S[["TSNCONJ_M"]][cl,] + rnorm(ncontext,0,0.5);
  ...
  # signal (Adjust this part!)
  obs <- cbind(rule_s,noise); 
}
...
results<-
  foreach(cc=clist) %dopar% {
    ...
    # TNSCONJ (Rule x Resp/Stim x Next) with CJ RT control
    rsaext <- ""
    #predF[, (paste0('RULE_RSA',rsaext)) := RULE_M[,cc]]
    predF[, (paste0('STIM_RSA',rsaext)) := STIMPOS_M[,cc]]
    predF[, (paste0('RESP_RSA',rsaext)) := RESP_M[,cc]]
    predF[, (paste0('RESPN_RSA',rsaext)) := RESPN_M[,cc]]
    predF[, (paste0('CONG_RSA',rsaext)) := CONG_M[,cc]]
    predF[, (paste0('TSCONJ_RSA',rsaext)) := TSCONJ_M[,cc]]
    predF[, (paste0('TSNCONJ_RSA',rsaext)) := TSNCONJ_M[,cc]]
