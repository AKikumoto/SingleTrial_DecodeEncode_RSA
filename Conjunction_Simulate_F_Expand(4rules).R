...
Dir_RSAMODEL<-file.path(Dir_HERE,"PTSRCONJ_E")
...
results<-
  foreach(cc=clist) %dopar% {
    ...
    predF$STIM_RSA<-STIMPOS_M[,cc];#STIM MODEL
    predF$RESP_RSA<-RESP_M[,cc];#RESP MODEL
    predF$ODDEVEN_RSA<-ODDEVEN_M[,cc];#ODDEVEN MODEL
    predF$SRCONJ_RSA<-SRCONJ_M[,cc];#stim-resp conjunction
    predF$RSRCONJ_RSA<-RSRCONJ_M[,cc];#rule-stim-resp MODEL
    predF$RSRCCONJ_RSA<-RSRCCONJ_M[,cc];# highest level (should be biased)
